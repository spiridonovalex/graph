...
public class GraphOnMap<V, E> implements Graph<V, E> {	
	...
	public GraphOnMap(boolean directed) {
		this.directed = directed;
	}
	
	@Override
	public void forEachVertex(Consumer<V> consumer) {
		vertices.keySet().forEach(consumer);
	}
	...
	public List<Edge<V,E>> getPath(V from, V to) {
		...
		if (from.equals(to)) {
			var edge = this.vertices.get(from).get(to);
		...
		while (true) {
			...
			var path = toExtend.intersects(toCheck);
			if (path != null) {
				return path;
			}
		}
